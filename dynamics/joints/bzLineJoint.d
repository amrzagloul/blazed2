...
*/
module blaze.dynamics.joints.bzLineJoint;
...
class bzLineJointDef : bzJointDef
{
	this(bzBody body1, bzBody body2, bzVec2 anchor = bzVec2(0,0),
         bzVec2 axis = bzVec2(0,0)) {

		super(body1, body2);
		localAnchor1 = body1.localPoint(anchor);
		localAnchor2 = body2.localPoint(anchor);
		localAxis1 = body1.localVector(axis);
		type = bzJointType.LINE;
...
class bzLineJoint : bzJoint
{

	this(bzLineJointDef def) {
		...
		m_axis = bzVec2.zeroVect;
		m_perp = bzVec2.zeroVect;
	}
	...
	//--------------- Internals Below -------------------
		...
		if (step.warmStarting)
		{
			...
			bzVec2 P = m_impulse.x * m_perp + (m_motorImpulse + m_impulse.y) * m_axis;
	...
	override void solveVelocityConstraints(bzTimeStep step) {
		...
		float Cdot1 = bzDot(m_perp, v2 - v1) + m_s2 * w2 - m_s1 * w1;

		if (m_enableLimit && m_limitState != bzLimitState.INACTIVE)
		{
			// solve prismatic and limit constraint in block form.
			float Cdot2 = bzDot(m_axis, v2 - v1) + m_a2 * w2 - m_a1 * w1;
			bzVec2 Cdot = bzVec2(Cdot1, Cdot2);

			bzVec2 f1 = m_impulse;
			bzVec2 df =  m_K.solve(-Cdot);
			m_impulse += df;
			...
			bzVec2 P = df.x * m_perp + df.y * m_axis;
	...
	override bool solvePositionConstraints(float baumgarte) {
		...
		linearError = max(linearError, cast(float) abs(C1));
		angularError = 0.0f;
		...
		bzVec2 P = impulse.x * m_perp + impulse.y * m_axis;
